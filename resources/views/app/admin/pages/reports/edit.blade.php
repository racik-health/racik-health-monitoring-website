...
@section('meta-description',
    'Edit laporan konsumsi jamu pasien di Racik Health. Perbarui catatan konsumsi jamu pasien
    untuk pemantauan kesehatan terintegrasi.')
@section('meta-keywords',
    'edit laporan konsumsi, ubah catatan konsumsi jamu, update rekam medis digital, sistem
    monitoring pasien, dashboard tenaga medis')
@section('og-title', 'Manajemen Laporan Konsumsi - Racik')
@section('og-description', 'Formulir pengeditan konsumsi jamu pasien dalam sistem Racik Health.')
        ...
        <div class="bg-white rounded-xl shadow-xs p-4 sm:p-7">
            <div class="mb-8">
                ...
                <p class="text-sm text-gray-600">
                    Ubah data laporan konsumsi baru
                </p>
            </div>
            <form method="POST" action="{{ route('admin.consumption-reports.update', $data['consumption_report']->id) }}">
                @csrf
                @method('PUT')
                <div class="grid sm:grid-cols-12 gap-2 sm:gap-6">
                        ...
                        <select id="user_id" name="user_id" required
                            ...
                            @foreach ($data['patients'] as $patient)
                                <option value="{{ $patient->id }}"
                                    {{ old('user_id', $data['consumption_report']->user_id) == $patient->id ? 'selected' : '' }}>
                                    {{ $patient->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select id="herbal_medicine_id" name="herbal_medicine_id" required
                            ...
                            @foreach ($data['herbal_medicines'] as $herbalMedicine)
                                <option value="{{ $herbalMedicine->id }}"
                                    {{ old('herbal_medicine_id', $data['consumption_report']->herbal_medicine_id) == $herbalMedicine->id ? 'selected' : '' }}>
                                    {{ $herbalMedicine->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="sm:col-span-9">
                        <input id="quantity" type="number"
                            class="py-1.5 sm:py-2 px-3 pe-11 block w-full border-gray-200 shadow-2xs sm:text-sm rounded-lg focus:border-blue-500 focus:ring-blue-500 checked:border-blue-500 disabled:opacity-50 disabled:pointer-events-none"
                            name="quantity" placeholder="2"
                            value="{{ old('quantity', $data['consumption_report']->quantity) }}" required>
                        @error('quantity')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="sm:col-span-9">
                        <input id="note" type="text"
                            class="py-1.5 sm:py-2 px-3 pe-11 block w-full border-gray-200 shadow-2xs sm:text-sm rounded-lg focus:border-blue-500 focus:ring-blue-500 checked:border-blue-500 disabled:opacity-50 disabled:pointer-events-none"
                            name="note" placeholder="Catatan singkat tentang konsumsi jamu"
                            value="{{ old('note', $data['consumption_report']->note) }}" required>
                        @error('note')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="sm:col-span-9">
                        <input id="consumed_at" type="datetime-local"
                            class="py-1.5 sm:py-2 px-3 pe-11 block w-full border-gray-200 shadow-2xs sm:text-sm rounded-lg focus:border-blue-500 focus:ring-blue-500 checked:border-blue-500 disabled:opacity-50 disabled:pointer-events-none"
                            name="consumed_at" placeholder="YYYY-MM-DDTHH:MM"
                            value="{{ old('consumed_at', $data['consumption_report']->consumed_at) }}" required>
                        @error('consumed_at')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
